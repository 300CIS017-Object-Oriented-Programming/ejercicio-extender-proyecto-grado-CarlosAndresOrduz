...
class ControladorPdf:
    ...
    def exportar_acta(self, st, controlador, acta_seleccionada):
        ...
        self.pdf.cell(200, 10, txt="Trabajo", ln=1, align='C')
        ...
        for acta in controlador.actas:
            if acta.autor == acta_seleccionada:
                ...
                for criterio in acta.criterios:
                    ...
                    self.pdf.set_font("times", size=12)
                    self.pdf.cell(200, 10, txt=f"Calificación parcial: {round(criterio.nota, 2)}", ln=1, align='L')
                    self.pdf.cell(200, 10, txt=f"Ponderación: {criterio.porcentaje * 100}%", ln=1, align='L')
                    self.pdf.cell(200, 10, txt=f"Observación: {criterio.observacion}", ln=1,align='L')
                    self.pdf.cell(200, 10, txt=f"Observaciones adicionales: {criterio.observacionAdicional}", ln=1,align='L')
                    self.pdf.cell(200, 10, txt=f"Restricciones: {criterio.restricciones}", ln=1,align='L')
                    numero += 1
                self.pdf.set_font("times", 'B', size=12)
                self.pdf.multi_cell(185, 10,
                               ...
                               align='L')
                self.pdf.cell(200, 10, txt=f"{round(acta.nota_final, 2)}", ln=0, align='C')
                self.pdf.cell(200, 10, txt=f"{controlador.mostrar_de_numero_a_palabras(acta.nota_final)}", ln=1, align='C')
                self.pdf.cell(200, 10, txt="Números", ln=0, align='C')
                self.pdf.cell(200, 10, txt="Letras", ln=1, align='C')
                self.pdf.set_font("times", size=12)
                self.pdf.cell(200, 10, txt="Observaciones adicionales: ", ln=1,align='L')
                self.pdf.cell(200, 10,
                ...
                self.pdf.cell(200, 10,
                               ...
                               ln=1,
                               align='L')
                self.pdf.cell(200, 10, txt="________________________________   ________________________________", ln=2,
                         align='C')
                self.pdf.cell(200, 10, txt=f"{acta.jurado1}                                    {acta.jurado2} ", ln=2,
                         align='C')
